...
export default function Register() {
  ...
  return (
    <>
      <Helmet>
        <title>Create Account — {config.hotelName}</title>
      </Helmet>
      ...
      <div className="flex min-h-screen items-center justify-center bg-ink px-6">
        <div className="w-full max-w-sm rounded-2xl border border-warm-white/10 bg-white/[0.03] p-8 backdrop-blur">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-gold/10">
            <UserPlus className="h-5 w-5 text-gold" strokeWidth={1.5} />
          </div>
          <h1 className="mt-5 text-center font-display text-2xl text-warm-white">{config.hotelName}</h1>
          <p className="mt-1 text-center text-xs uppercase tracking-wider text-gold-light">Create Account</p>

          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            <label className="flex flex-col gap-1">
              <span className="text-[11px] font-medium uppercase tracking-wider text-warm-white/60">Full Name</span>
              <input
                type="text"
                value={name}
          ...
          <p className="mt-6 text-center text-xs text-warm-white/50">
            ...
            <Link to="/admin/login" className="text-gold-light underline">
              Sign in
            </Link>
          </p>
        </div>
